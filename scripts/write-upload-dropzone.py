...
lines = [
    ...
    "      setMessage(",
    '        locale === "vi" ? "Chỉ chấp nhận tệp ảnh." : "Images only.",',
    "      );",
    "      return;",
    "    }",
    "",
    ...
    "      setMessage(",
    '        locale === "vi"',
    '          ? "Đã nhận ảnh. Sẵn sàng quét."',
    '          : "Image received. Ready to scan.",',
    "      );",
    "    };",
    ...
    "      setMessage(",
    '        locale === "vi"',
    '          ? "Hãy chọn ảnh trước khi quét."',
    '          : "Choose an image before scanning.",',
    "      );",
    "      return;",
    "    }",
    "",
    ...
    "",
    "  return (",
    f'    {DIV_O} className="mx-auto grid w-full max-w-5xl grid-cols-1 gap-8 lg:grid-cols-[1fr_0.72fr]">',
    "      <section",
    ...
    "        }}",
    "      >",
    f'        {DIV_O} className="absolute inset-6 rounded-lg border border-dashed border-gold/30" />',
    f'        {DIV_O} className="relative flex h-full min-h-[28rem] flex-col items-center justify-center gap-6 text-center">',
    "          {preview ? (",
    f'            {DIV_O} className="relative h-64 w-full overflow-hidden rounded-lg border border-white/10 sm:h-80">',
    "              <Image",
    "                src={preview}",
    '                alt=""',
    "                fill",
    ...
    f"            {DIV_C}",
    "          ) : (",
    f'            {DIV_O} className="grid size-24 place-items-center rounded-lg border border-gold/30 bg-gold/10 text-gold">',
    ...
    "            </p>",
    f"          {DIV_C}",
    f'          {DIV_O} className="flex flex-col gap-3 sm:flex-row">',
    "            <input",
    "              ref={fileInputRef}",
    '              className="hidden"',
    '              type="file"',
    ...
    f"        {DIV_O}>",
    f'          {DIV_O} className="grid size-12 place-items-center rounded-lg border border-sage/40 bg-sage/15 text-sage">',
    ...
    f'          {DIV_O} className="mt-6 grid grid-cols-5 gap-2">',
...
Path("src/components/upload-dropzone.tsx").write_text("\n".join(lines), encoding="utf-8")
print("ok", DIV_C)
